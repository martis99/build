...
static int gen_source(const proj_t *proj, const dict_t *projects, const prop_t *sln_props, resolve_fn resolve, resolve_path_fn resolve_path, pgc_t *gen)
{
	...
	gen->str[PGC_STR_NAME] = str_cpy(*filename);

	switch (proj->props[PROJ_PROP_TYPE].mask) {
	case PROJ_TYPE_EXE:
		gen->builds = F_PGC_BUILD_EXE;
		break;
	case PROJ_TYPE_BIN:
		gen->builds = F_PGC_BUILD_BIN;
		break;
	case PROJ_TYPE_ELF:
		gen->builds = F_PGC_BUILD_ELF;
		break;
	case PROJ_TYPE_FAT12:
		gen->builds = F_PGC_BUILD_FAT12;
		break;
	case PROJ_TYPE_LIB:
		gen->builds = F_PGC_BUILD_STATIC | F_PGC_BUILD_SHARED;
		break;
	case PROJ_TYPE_EXT:
		break;
	}
	...
	if (outdir->flags & PROP_SET) {
		gen->str[PGC_STR_OUTDIR] = str_cpy(resolve(outdir->value.val, &buf, proj));
		switch (proj->props[PROJ_PROP_TYPE].mask) {
		case PROJ_TYPE_EXE: {
			str_t c = strn(buf.data, buf.len, buf.len + 5);
			str_cat(&c, STR("cov/"));
			gen->str[PGC_STR_COVDIR] = c;
			break;
		}
		default:
			break;
	...
	if (proj->props[PROJ_PROP_SOURCE].arr.cnt > 0) {
		...
		if (intdir->flags & PROP_SET) {
			...
			case PROJ_TYPE_BIN:
			case PROJ_TYPE_ELF:
				gen->intdir[PGC_INTDIR_STR_INTDIR][PGC_INTDIR_OBJECT] = str_cpy(buf);
				break;
			case PROJ_TYPE_LIB: {
				resolve(intdir->value.val, &buf, proj);
				str_t s = strn(buf.data, buf.len, buf.len + 8);
				str_cat(&s, STR("static/"));
				gen->intdir[PGC_INTDIR_STR_INTDIR][PGC_INTDIR_STATIC] = s;

				str_t d = strn(buf.data, buf.len, buf.len + 8);
				str_cat(&d, STR("shared/"));
				gen->intdir[PGC_INTDIR_STR_INTDIR][PGC_INTDIR_SHARED] = d;
				break;
			}
			default:
				break;
	...
	{
		pgc_add_config(gen, strs(config->val));
	}
	...
	arr_foreach(&proj->props[PROJ_PROP_INCLUDE].arr, include)
	{
		...
		if (exts) {
			pgc_add_header(gen, strs(include->val), exts);
		}
	}
	...
	arr_foreach(&proj->props[PROJ_PROP_SOURCE].arr, source)
	{
		...
		if (headers) {
			pgc_add_header(gen, strs(source->val), headers);
		}
		...
		if (srcs) {
			pgc_add_src(gen, strs(source->val), srcs);
		}
	}
	...
	if (partifact->flags & PROP_SET) {
		...
		gen->target[PGC_TARGET_STR_ARTIFACT][PGC_BUILD_EXE]    = strs(partifact->value.val);
		gen->target[PGC_TARGET_STR_ARTIFACT][PGC_BUILD_BIN]    = strs(partifact->value.val);
		gen->target[PGC_TARGET_STR_ARTIFACT][PGC_BUILD_ELF]    = strs(partifact->value.val);
		gen->target[PGC_TARGET_STR_ARTIFACT][PGC_BUILD_FAT12]  = strs(partifact->value.val);
		gen->target[PGC_TARGET_STR_ARTIFACT][PGC_BUILD_STATIC] = strs(partifact->value.val);
		gen->target[PGC_TARGET_STR_ARTIFACT][PGC_BUILD_SHARED] = strs(partifact->value.val);
	}

	if (proj->props[PROJ_PROP_ARGS].flags & PROP_SET) {
		gen->str[PGC_STR_ARGS] = str_cpy(resolve(proj->props[PROJ_PROP_ARGS].value.val, &buf, proj));
	}

	arr_foreach(&proj->props[PROJ_PROP_SOURCE].arr, source)
	{
		pgc_add_include(gen, str_cpy(resolve(source->val, &buf, proj)));
	}

	arr_foreach(&proj->props[PROJ_PROP_INCLUDE].arr, include)
	{
		pgc_add_include(gen, str_cpy(resolve(include->val, &buf, proj)));
	}
	...
	for (uint i = 0; i < proj->includes.cnt; i++) {
		...
		{
			pgc_add_include(gen, str_cpy(resolve(resolve_path(rel, include->val, &buf), &buf, iproj)));
		}

		if (iproj->props[PROJ_PROP_ENCLUDE].flags & PROP_SET) {
			pgc_add_include(gen, str_cpy(resolve(resolve_path(rel, iproj->props[PROJ_PROP_ENCLUDE].value.val, &buf), &buf, iproj)));
		}
	}
	...
	if (proj->props[PROJ_PROP_DEFINES].flags & PROP_SET) {
		...
		for (uint k = 0; k < defines->cnt; k++) {
			...
			pgc_add_define(gen, resolve(define->val, &buf, proj), F_PGC_INTDIR_OBJECT | F_PGC_BUILD_STATIC | F_PGC_INTDIR_SHARED);
		}
	}
	...
	if ((flags->flags & PROP_SET)) {
		if (flags->mask & (1 << FLAG_STD_C99)) {
			pgc_add_flag(gen, STR("-std=c99"), PGC_SRC_C);
		}
		if (flags->mask & (1 << FLAG_FREESTANDING)) {
			pgc_add_flag(gen, STR("-ffreestanding"), PGC_SRC_C);
		}
	}

	pgc_add_flag(gen, STR("-Wall -Wextra -Werror -pedantic"), F_PGC_SRC_C | F_PGC_SRC_CPP);
	...
	arr_foreach(&proj->all_depends, dep)
	{
		...
		pgc_add_define(gen, define, F_PGC_INTDIR_OBJECT | F_PGC_INTDIR_SHARED);
		str_free(&define);
	}
	...
	pgc_add_define(gen, define, F_PGC_INTDIR_SHARED);
	str_free(&define);
	...
	arr_foreach(&proj->depends, dep)
	{
		if (!(dep->proj->props[PROJ_PROP_SOURCE].flags & PROP_SET) || dep->link_type == LINK_TYPE_SHARED) {
			if (dep->link_type == LINK_TYPE_STATIC) {
				pgc_add_depend(gen, strf("%.*s_s", dep->proj->name.len, dep->proj->name.data));
			} else {
				pgc_add_depend(gen, strf("%.*s_d", dep->proj->name.len, dep->proj->name.data));
			}
		}
	}
	...
	if (!(proj->props[PROJ_PROP_SOURCE].flags & PROP_SET)) {
		arr_foreach(&proj->depends, dep)
		{
			if (dep->proj->props[PROJ_PROP_TYPE].mask == PROJ_TYPE_LIB) {
				...
				pgc_add_depend(gen, str_cpy(dep->proj->name));
			}
		}
	}
	...
	if (flags->flags & PROP_SET) {
		if (flags->mask & (1 << FLAG_WHOLEARCHIVE)) {
			pgc_add_ldflag(gen, STR("-Wl,--whole-archive"));
		}
		if (flags->mask & (1 << FLAG_ALLOWMULTIPLEDEFINITION)) {
			pgc_add_ldflag(gen, STR("-Wl,--allow-multiple-definition"));
		}
	}
	...
	for (int i = proj->all_depends.cnt - 1; i >= 0; i--) {
			...
			if (dep->proj->props[PROJ_PROP_SOURCE].flags & PROP_SET) {
				...
				if (doutdir->flags & PROP_SET) {
					...
					if (dep->link_type == LINK_TYPE_SHARED) {
						pgc_add_lib(gen, str_cpy(buf), str_cpy(dep->proj->name), PGC_LINK_SHARED, PGC_LIB_INT);
						str_cat(&buf, dep->proj->name);
						str_cat(&buf, STR(".so"));
						pgc_add_copyfile(gen, str_cpy(buf));
					} else {
						pgc_add_lib(gen, str_cpy(buf), str_cpy(dep->proj->name), PGC_LINK_STATIC, PGC_LIB_INT);
					}
				}
			} else {
				...
				if (dep->link_type == LINK_TYPE_STATIC && lib->flags & PROP_SET) {
					...
					pgc_add_lib(gen, str_cpy(buf), strs(lib->value.val), PGC_LINK_STATIC, PGC_LIB_EXT);
				}

				const prop_t *dlib = &dep->proj->props[PROJ_PROP_DLIB];
				if (dep->link_type == LINK_TYPE_SHARED && dlib->flags & PROP_SET) {
					resolve(resolve_path(rel, STR(""), &buf), &buf, dep->proj);
					pgc_add_lib(gen, str_cpy(buf), str_cpy(dlib->value.val), PGC_LINK_SHARED, PGC_LIB_EXT);
					resolve(resolve_path(rel, dlib->value.val, &buf), &buf, dep->proj);
					pgc_add_copyfile(gen, strf("%.*s.so", buf.len, buf.data));
				}
			}
		}
		...
		arr_foreach(&dep->proj->props[PROJ_PROP_LIBDIRS].arr, libdir)
		{
			...
			pgc_add_lib(gen, str_cpy(buf), str_null(), PGC_LINK_SHARED, PGC_LIB_EXT);
		}
		...
		{
			pgc_add_lib(gen, str_null(), str_cpy(link->val), PGC_LINK_SHARED, PGC_LIB_EXT);
		}
	}

	if ((flags->flags & PROP_SET) && (flags->mask & (1 << FLAG_WHOLEARCHIVE))) {
		pgc_add_ldflag(gen, STR("-Wl,--no-whole-archive"));
	}

	if (lang & (1 << LANG_CPP)) {
		pgc_add_ldflag(gen, STR("-lstdc++"));
	}

	//TODO: Think of better solution
	if (flags->flags & PROP_SET) {
		if (flags->mask & (1 << FLAG_STATIC)) {
			pgc_add_ldflag(gen, STR("-static"));
		}
		if (flags->mask & (1 << FLAG_NOSTARTFILES)) {
			pgc_add_ldflag(gen, STR("-nostartfiles"));
		}
		...
		if (flags->mask & (1 << FLAG_X11)) {
			pgc_add_ldflag(gen, STR("-lX11"));
		}

		if (flags->mask & (1 << FLAG_GL)) {
			pgc_add_ldflag(gen, STR("-lGL"));
		}

		if (flags->mask & (1 << FLAG_GLX)) {
			pgc_add_ldflag(gen, STR("-lGLX"));
		}
	}
	...
	{
		pgc_add_require(gen, str_cpy(require->val));
	}

	const prop_str_t *copyfile;
	arr_foreach(&proj->props[PROJ_PROP_COPYFILES].arr, copyfile)
	{
		pgc_add_copyfile(gen, str_cpy(resolve(copyfile->val, &buf, proj)));
	}
	...
	if (header->flags & PROP_SET) {
		...
		if (dict_get(projects, header->value.val.data, header->value.val.len, (void **)&fproj)) {
			...
			str_cat(&tmp, STR(".bin"));
			gen->str[PGC_STR_HEADER] = str_cpy(resolve(tmp, &buf, fproj));
		}
	}
	...
	arr_foreach(&proj->props[PROJ_PROP_FILES].arr, fname)
	{
		...
		case PROJ_TYPE_BIN:
			str_cat(&tmp, STR(".bin"));
			pgc_add_file(gen, str_cpy(resolve(tmp, &buf, fproj)), PGC_FILE_BIN);
			break;
		case PROJ_TYPE_ELF:
			str_cat(&tmp, STR(".elf"));
			pgc_add_file(gen, str_cpy(resolve(tmp, &buf, fproj)), PGC_FILE_ELF);
			break;
		}
	}
	...
	if (size->flags & PROP_SET) {
		gen->str[PGC_STR_SIZE] = str_cpy(size->value.val);
	}

	const prop_t *run = &proj->props[PROJ_PROP_RUN];
	if (run->flags & PROP_SET) {
		pgc_set_run(gen, str_cpy(resolve(run->value.val, &buf, proj)), gen->builds);
	}

	const prop_t *drun = &proj->props[PROJ_PROP_DRUN];
	if (drun->flags & PROP_SET) {
		pgc_set_run_debug(gen, str_cpy(resolve(drun->value.val, &buf, proj)), gen->builds);
	}

	return 0;
}

static int gen_url(const proj_t *proj, resolve_fn resolve, resolve_path_fn resolve_path, pgc_t *gen)
{
	char buf_d[P_MAX_PATH] = { 0 };

	str_t buf = strb(buf_d, sizeof(buf_d), 0);

	gen->str[PGC_STR_NAME] = str_cpy(proj->name);

	if (proj->props[PROJ_PROP_URL].flags & PROP_SET) {
		gen->str[PGC_STR_URL] = str_cpy(proj->props[PROJ_PROP_URL].value.val);
	}

	if (proj->props[PROJ_PROP_FORMAT].flags & PROP_SET) {
		gen->str[PGC_STR_FORMAT] = str_cpy(proj->props[PROJ_PROP_FORMAT].value.val);
	}

	if (proj->props[PROJ_PROP_OUTDIR].flags & PROP_SET) {
		gen->str[PGC_STR_OUTDIR] = str_cpy(resolve(proj->props[PROJ_PROP_OUTDIR].value.val, &buf, proj));
	}
	...
	if (proj->props[PROJ_PROP_CONFIG].flags & PROP_SET) {
		gen->str[PGC_STR_CONFIG] = str_cpy(proj->props[PROJ_PROP_CONFIG].value.val);
	}

	if (proj->props[PROJ_PROP_TARGET].flags & PROP_SET) {
		gen->str[PGC_STR_TARGETS] = str_cpy(proj->props[PROJ_PROP_TARGET].value.val);
	}
...
{
	if (proj->props[PROJ_PROP_URL].flags & PROP_SET) {
		return gen_url(proj, resolve, resolve_path, gen);
	}

	return gen_source(proj, projects, sln_props, resolve, resolve_path, gen);
}
