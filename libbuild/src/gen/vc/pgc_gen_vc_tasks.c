...
static json_t *pgc_gen_vc_task(const pgc_t *pgc, json_t *json, json_val_t tasks, pgc_build_type_t b, str_t arch, str_t config)
{
	// clang-format off
	static struct {
		str_t act;
	...
	if ((str_eq(config, STR("Debug")) && pgc->target[PGC_TARGET_STR_RUN_DBG][b].data) || pgc->target[PGC_TARGET_STR_RUN][b].data || b == PGC_BUILD_EXE) {
