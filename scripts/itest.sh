...
mk_check() {
	...
	if ! make -C itests ARCH="$1" CONFIG="$2" >itests/tmp/build/mk/"$1"-"$2"/make.log 2>&1; then
		printf "\033[0;31m[Make] Failed to build %s-%s\033[0m\n" "$1" "$2"
		cat itests/tmp/build/mk/"$1"-"$2"/make.log
		ret=1
		return
	fi
...
DLIBCPP: test"
	...
	if ! make $make_flags -C itests fat12/run_fat12 ARCH="$1" CONFIG="$2" >itests/tmp/logs/"$1"-"$2"/projects/fat12/run.log 2>&1; then
		printf "\033[0;31mFAILED\033[0m\n"
		cat itests/tmp/logs/"$1"-"$2"/projects/fat12/run.log
		ret=1
	else
		printf "\033[0;32mOK\033[0m\n"
	fi
...
cm_mk_build() {
	...
	if ! cmake -S itests -B itests/tmp/build/cm_mk/"$1"-"$2" -G "Unix Makefiles" -DARCH="$1" -DCMAKE_BUILD_TYPE="$2" >itests/tmp/build/cm_mk/"$1"-"$2"/cmake.log 2>&1; then
		printf "\r\033[0;31m[CMake] Failed to generate Makefiles\033[0m\n"
		cat itests/tmp/build/cm_mk/"$1"-"$2"/cmake.log
		ret=1
		return
	fi
}
...
cm_mk_check() {
	mkdir -p itests/tmp/build/cm_mk/"$1"-"$2"
	if ! make -C itests/tmp/build/cm_mk/"$1"-"$2" >itests/tmp/build/cm_mk/"$1"-"$2"/make.log 2>&1; then
		printf "\033[0;31m[CMake] [Make] Failed to build %s-%s\033[0m\n" "$1" "$2"
		cat itests/tmp/build/cm_mk/"$1"-"$2"/make.log
		ret=1
		return
	fi
...
if ! $build -S itests -G M $build_flags >itests/tmp/build/mk/build.log; then
	printf "\033[0;31mFailed to generate Make files\033[0m\n"
	cat itests/tmp/build/mk/build.log
	ret=1
else
...
printf "[Build] Generating CMakefiles\n"
mkdir -p itests/tmp/build/cm_mk
# shellcheck disable=SC2086
if ! $build -S itests -G C $build_flags >itests/tmp/build/cm_mk/build.log 2>&1; then
	printf "\033[0;31m[Build] Failed to generate CMake files\033[0m\n"
	cat itests/tmp/build/cm_mk/build.log
	ret=1
else
