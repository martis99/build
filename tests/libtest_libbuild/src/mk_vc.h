...
static const char *SLN_TEST = "SLNDIR := $(CURDIR)\n"
			      ...
			      "SHOW := true\n"
			      "\n"
			      ".PHONY: all check test/clean test/compile test/run test/coverage clean\n"
			      "\n"
			      "all: test/compile\n"
			      "\n"
			      "check:\n"
...
static const char *OS_SLN =
	...
	"SHOW := true\n"
	"\n"
	".PHONY: all check boot-bin/clean boot-bin/compile gcc-13.1.0/clean gcc-13.1.0/compile kernel-bin/clean kernel-bin/compile kernel-bin/coverage kernel-elf/clean kernel-elf/compile kernel-elf/coverage image-disk/clean image-disk/compile image-disk/run image-disk/artifact image-floppy/clean image-floppy/compile image-floppy/run image-floppy/artifact artifact clean\n"
	"\n"
	"all: boot-bin/compile gcc-13.1.0/compile kernel-bin/compile kernel-elf/compile image-disk/compile image-floppy/compile\n"
	"\n"
	"check:\n"
	"ifeq ($(filter $(CONFIG),$(CONFIGS)),)\n"
	"\t$(error Config '$(CONFIG)' not found. Configs: $(CONFIGS))\n"
	...
	"\n"
	"kernel-bin/coverage: check\n"
	"\t@$(MAKE) -C os/kernel/bin coverage\n"
	"\n"
	...
	"\n"
	"kernel-elf/coverage: check\n"
	"\t@$(MAKE) -C os/kernel/elf coverage\n"
	"\n"
	"image-disk/clean: check boot-bin/clean kernel-bin/clean kernel-elf/clean\n"
	"\t@$(MAKE) -C os/image/disk clean\n"
	"\n"
	"image-disk/compile: check boot-bin/compile kernel-bin/compile kernel-elf/compile\n"
	...
	"\n"
	"image-floppy/clean: check boot-bin/clean kernel-bin/clean kernel-elf/clean\n"
	"\t@$(MAKE) -C os/image/floppy clean\n"
	"\n"
	"image-floppy/compile: check boot-bin/compile kernel-bin/compile kernel-elf/compile\n"
...
static const char *OS_PROJ_KERNEL_BIN = "OUTDIR := $(SLNDIR)/bin/$(CONFIG)-$(PLATFORM)/os/kernel/bin/\n"
					...
					"CFLAGS += $(FLAGS) -Wall -Wextra -Werror -pedantic -ffreestanding -m$(BITS)\n"
					"LDFLAGS +=\n"
					"\n"
					"RM += -r\n"
					"\n"
					...
					"endif\n"
					"\n"
					".PHONY: all check check_coverage compile coverage clean\n"
					"\n"
					"all: compile\n"
					"\n"
					"check:\n"
					...
					"\n"
					"clean:\n"
					"\t@$(RM) $(TARGET) $(OBJ_ASM) $(OBJ_C) $(OBJ_D_C) $(COV)\n";
