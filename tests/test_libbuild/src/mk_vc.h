...
static const char *OS_SLN =
	...
	"SHOW := true\n"
	"\n"
	".PHONY: all check boot boot/clean boot/compile boot/coverage image image/clean image/compile image/run image/coverage kernel kernel/clean kernel/compile kernel/coverage gcc-13.1.0 gcc-13.1.0/clean gcc-13.1.0/compile gcc-13.1.0/coverage clean\n"
	"\n"
	"all: boot image kernel gcc-13.1.0\n"
	"\n"
	"check:\n"
	"ifeq ($(filter $(CONFIG),$(CONFIGS)),)\n"
	"\t$(error Config '$(CONFIG)' not found. Configs: $(CONFIGS))\n"
