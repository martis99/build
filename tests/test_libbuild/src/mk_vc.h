...
static const char *OS_PROJ_IMAGE_DISK = "OUTDIR := $(SLNDIR)/bin/$(CONFIG)-$(PLATFORM)/os/image/disk/\n"
					...
					"endif\n"
					"\n"
					".PHONY: all check image-disk compile run clean\n"
					"\n"
					"all: image-disk\n"
					"\n"
					"check:\n"
					...
					"$(TARGET):\n"
					"\t@mkdir -p $(@D)\n"
					"\t@dd if=$(SLNDIR)/bin/$(CONFIG)-$(PLATFORM)/os/boot/bin/boot.bin >> $@\n"
					"\t@dd if=$(SLNDIR)/bin/$(CONFIG)-$(PLATFORM)/os/kernel/bin/kernel.bin >> $@\n"
					"\n"
					"run: $(TARGET)\n"
					"ifeq ($(CONFIG), Debug)\n"
...
static const char *OS_PROJ_IMAGE_FLOPPY =
	...
	"endif\n"
	"\n"
	".PHONY: all check image-floppy compile run clean\n"
	"\n"
	"all: image-floppy\n"
	"\n"
	"check:\n"
